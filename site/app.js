...
async function boot() {
  ...
  if (!state.catalog.publication || !['generic', 'public_personalization'].includes(state.catalog.publication.mode)) {
    throw new Error('公開範囲の設定が見つかりません。文字を表示する前に公開データを確認してください。');
  }
  $('#message-line-1').textContent = state.catalog.message.lines[0];
  $('#message-line-2').textContent = state.catalog.message.lines[1];
  $('#message-privacy-note').textContent = state.catalog.publication.mode === 'generic'
    ? 'PLACEHOLDER / 実在の個人プロフィールではありません'
    : '公開承認済みの表示文字 / このページと生成物に公開されます';
  try {
