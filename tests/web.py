...
with sync_playwright() as playwright:
    ...
    expect(page.locator("body")).to_contain_text("実機印刷して合格した証拠はまだありません")
    expect(page.locator('a[href*="downloads/lettering-coupons.zip"]')).to_have_count(1)
    assert page.locator(".document img").evaluate_all("images => images.every(image => image.complete && image.naturalWidth > 0)")
