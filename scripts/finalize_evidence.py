...
def main():
    ...
    result = {
        ...
        "final_browser_recheck": {"status": "PASS_CURRENT_REVISION_CI" if current_browser else "PENDING_CURRENT_REVISION_CI",
                                  "revision": c["revision"],
                                  "required_gate": "Current-revision full Chrome browser checks before Pages deployment; not physical validation."},
        "unselected_individual_variant": "withheld from public distribution",
        "private_personalization_route": private_route,
        "lettering_revision": read("validation/lettering.json"),
        "lettering_trial_coupons": read("validation/lettering-coupons.json"),
        "plaque_only_render_intervals": [read(f"validation/plaque-render-{model['id']}.json") for model in c["models"]],
