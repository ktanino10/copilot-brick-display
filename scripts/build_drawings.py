...
def sheet(title, subtitle, content):
    return (
        ...
        + text(42, 42, "BRICK PORTRAIT / DIGITAL PROTOTYPE", 12, GRAY)
...
def fragment(path, stroke=.18, color=INK):
    ...
    value = value.replace('stroke-width="1.0"', f'stroke-width="{stroke}"')
    return value.replace('stroke="rgb(0, 0, 0)"', f'stroke="{color}"')
...
def part_sheet(part, c):
    ...
    if part["kind"] == "dock":
        info += ["Slot: 88.40 x 2.40 x 4.80 deep", "Slot center: x=48, y=4", "Underside depth: 2.20"]
    if part["kind"] == "card":
        info += ["Flat back on bed / lettering up", "Body 2.00 / text relief 0.60", "Bottom 4.80 mm: no lettering"]
...
def exploded_sheet(model, metadata):
    ...
    content = projection(ROOT / "build/projections" / model["id"] / "exploded.svg",
                         "front", 180, 772, scale, model["bounds"][0][0], .16)
    content += text(800, 199, "EXPLODED FRONT", 18, weight="bold")
    for index, value in enumerate([
        "Actual solids, displaced only for this view.",
        "Each stage moves +6.4 mm x (stage-1) in Z.",
        "Do not measure assembly height here.",
        "Use overview for final outside dimensions.",
        "Insert parts downward in numbered order.",
        "External module shown separately below.",
        "Stud engagement != verified physical clutch.",
    ]):
        content += text(800, 239 + 29 * index, value, 12, GRAY)
    content += text(800, 495, "MSG-CARD / flat printing view", 13, weight="bold")
    content += projection(ROOT / "build/projections/parts/MSG-CARD/top.svg",
                          "top", 800, 600, 3.2)
    content += text(800, 665, "MSG-DOCK / front", 13, weight="bold")
    content += projection(ROOT / "build/projections/parts/MSG-DOCK/front.svg",
                          "front", 800, 748, 3.2)
...
def stage_sheet(model, step, c):
    ...
    if step["number"] == 1 and model["id"] == "C":
        ...
        content += text(795, 592, "Next dock ties across the center seam.", 12, BLUE)
...
def bom_sheet(model, rows, page_number):
    ...
    content += text(55, 770, f"Total assembly: {model['part_count']} pieces (dock + card included). Coupons are separate.", 13, BLUE)
...
def interface_sheet(c):
    ...
    lines = [
        ...
        "Thin plate and dock: nominal headroom 0.40",
...
def main():
    ...
    pdf_from_svgs([interface], ROOT / "site/downloads/interface.pdf")
    for model in c["models"]:
        folder = output / model["id"]
        pages = [save(folder / "overview.svg", model_overview(model)),
                 save(folder / "exploded.svg", exploded_sheet(model, metadata)), interface]
